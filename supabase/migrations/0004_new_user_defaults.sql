-- ============================================================================
-- PANEL OSOBISTY - 0004_new_user_defaults.sql
-- Po rejestracji nowe konto dostaje: profil, domyslne nawyki, cytaty
-- i pule pytan do refleksji. Wszystko edytowalne w Ustawieniach.
--
-- SECURITY DEFINER, bo trigger leci w kontekscie zapisu do auth.users,
-- gdzie auth.uid() jest jeszcze puste - wiersze wstawiamy z new.id.
-- ============================================================================
...
begin
  ...
  values
    (new.id, 'Wypiłem 2 litry wody',            1),
    (new.id, 'Ruch albo spacer 30 minut',        2),
    (new.id, 'Pierwsza godzina bez telefonu',    3),
    (new.id, 'Przeczytałem 10 stron',            4),
    (new.id, 'Spać przed 23:00',                 5);
...
-- Konta zalozone przed uruchomieniem tej migracji tez dostaja komplet.
insert into public.profiles (user_id)
