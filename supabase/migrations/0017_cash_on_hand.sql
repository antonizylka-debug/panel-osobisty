-- ============================================================================
-- PANEL OSOBISTY - 0017_cash_on_hand.sql
--
-- Gotowka w domu — ile fizycznie lezy w kopercie/szufladzie.
--
-- Model "spisu z natury", nie ruchow: kazdy wiersz zapisuje STAN po
-- przeliczeniu, a nie roznice. Powod jest praktyczny — gotowka rozchodzi
-- sie bez paragonow, wiec suma ruchow po miesiacu i tak rozjedzie sie z
-- rzeczywistoscia. Przeliczenie ("mam teraz 1240 zl") zawsze prostuje stan,
-- a historia zostaje.
--
-- Aktualny stan = wiersz o najpozniejszej dacie (przy remisie: ostatnio
-- dodany). Interfejs pozwala tez wpisac "dokladam 200" / "wyjmuje 50" —
-- przelicza to na nowy stan po swojej stronie i zapisuje jako spis.
--
...
create index if not exists cash_on_hand_user_date_idx
  on public.cash_on_hand (user_id, date desc, created_at desc);
...
begin
  ...
  raise notice 'OK - gotowka w domu gotowa.';
end
$do$;
